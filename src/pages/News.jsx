      ...
      <div className="w-[90%] mx-auto mt-14 flex items-start justify-between flex-wrap">
        ...
        <div className="w-full md:w-[60%] flex flex-wrap justify-center items-center">
          {listPerPage.map((item) => (
            <div
              key={item?.id}
              className="w-full flex items-start my-3 p-2 bg-gray-200 md:bg-white"
            >
              <div className="h-[300px] w-[400px] hidden lg:block relative overflow-hidden mr-4">
                <img
                  ...
                  alt=""
                  className="object-cover absolute w-full top-0 left-0 h-full cursor-pointer"
                />
              </div>
              <div className="w-full">
                <p className="bg-secondary px-4 py-2 text-white w-auto">
                  {item.name}
                </p>
                <div className="flex items-center w-full justify-start flex-wrap my-3">
                  <p className="flex mr-4 items-center text-[18px] my-2 text-[#666]">
                    <FaUserAlt className="text-secondary" />
                    &nbsp; {item.fullnameAuthor}
                  </p>
                  <p className="flex items-center text-[18px] my-2 text-[#666]">
                    <BsFillCalendarDateFill className="text-secondary" />
                    &nbsp; {item.creationDate}
                  </p>
                </div>
                <p
                  onClick={() => navigate(`/news/${item?.id}`)}
                  className="text-primary text-[24px] cursor-pointer hover:underline"
                >
                ...
                <div className="w-full flex justify-between items-center mt-5">
                  <button
                    onClick={() => navigate(`/news/${item?.id}`)}
                    className="btn-secondary px-2 md:px-5 py-1 md:py-2"
                  >
                    Đọc thêm
                  </button>
          ...
          <div className="w-full md:w-[50%] flex items-center justify-center">
            <button
              onClick={pre}
              className="text-[30px] text-primary mx-2 w-[40px] h-[40px] flex items-center justify-center"
            >
              <BsFillCaretLeftSquareFill />
            </button>
            {totalPage.map((item) => (
              <button
                ...
                key={item}
                className={`text-[20px] text-primary mx-2 w-[30px] h-[30px] flex items-center justify-center ${
                  currentPage == item ? "active-p" : ""
                }`}
              >
                {item}
              </button>
            ...
            {totalPage.length == 0 ? (
              <button
                className={`text-[20px] text-primary mx-2 w-[30px] h-[30px] flex items-center justify-center active-p`}
              >
                1
              </button>
            ) : null}
        ...
        <div className="w-full md:w-[38%] mt-24 md:mt-0">
          <div className="items-center w-full hidden md:flex">
            <input
              type="text"
              className="input-cus bg-[#eeeeee] h-[50px] px-4 w-[80%]"
              placeholder="Tìm kiếm tin tức"
            />
            <button className="btn-secondary h-[50px] w-[20%] flex justify-center items-center">
              <BiSearch className="" />
            </button>
          </div>

          <div className="w-full mt-5">
            <p className="text-primary text-[22px] md:text-[30px] font-bold">
              Kho
            </p>
            {listProviders.map((wh) => (
              <div
                key={wh.name}
                className="flex items-center justify-between my-4 text-[#666] text-[18px]"
              >
          ...
          <div className="w-full mt-10">
            <h1 className="text-[22px] lg:text-[30px] text-primary font-bold">
              Các bài đăng gần đây
            </h1>
            <div className="w-full my-5">
              {listNews.map((i2) => (
                <div
                  key={i2.namePostCategory}
                ...
                >
                  ...
                  <div className="w-full bg-gray-200 p-2 lg:bg-white lg:w-[70%] ml-0 md:ml-2">
                    <p
                      title={i2.name}
                      className="text-[20px] text-primary cursor-pointer hover:underline truncate-text"
                    >
                      {i2.name}
                    </p>
          ...
          <div className="w-full my-10">
            ...
            <div className="w-full flex justify-around items-center flex-wrap">
              {listHastag.map((tag) => (
                <button
                  className="my-2 px-4 py-2 text-primary border-gray-300 border-[1px] border-solid"
                  key={tag.id}
                >
                  {tag.hashtagName}
                </button>
