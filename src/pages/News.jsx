...
function News() {
  ...
  const listNews = [
    {
      author: 'Thuỳ Dung',
      title: 'Mẹo đóng gói & bảo quản đồ đạc khi dọn nhà',
      picture: 'https://www.wealthmanagement.com/sites/wealthmanagement.com/files/warehouse-demand.jpg',
      date: '14/02/2023 14:02:26',
      warehouse: 'King Kho'
    },
    {
      author: 'Quốc Dũng',
      title: 'Mẹo đóng gói & bảo quản đồ đạc khi lưu trữ',
      picture: 'https://www.wealthmanagement.com/sites/wealthmanagement.com/files/warehouse-demand.jpg',
      date: '14/02/2023 14:02:26',
      warehouse: 'King Kho'
    }
    ...
    <div className="w-full min-h-screen">
      ...
      <div className="w-[90%] mx-auto mt-14 flex items-start justify-between flex-wrap">
        ...
        <div className="w-full md:w-[60%] flex flex-wrap justify-center items-center">
          {listNews.map(item => (
            <div key={item.title} className="w-full flex items-start my-3 p-2 bg-gray-200 md:bg-white">
              <div className="h-[300px] w-[400px] hidden lg:block relative overflow-hidden mr-4">
                <img src={item.picture} alt="" className="object-cover absolute w-full h-full top-0 left-0" />
              </div>
              <div className='w-full'>
                <p className="bg-secondary px-4 py-2 text-white w-[140px]">{item.warehouse}</p>
                <div className='flex items-center w-full justify-start flex-wrap my-3'>
                  <p className='flex mr-4 items-center text-[18px] my-2 text-[#666]'><FaUserAlt className='text-secondary' />&nbsp; {item.author}</p>
                  <p className='flex items-center text-[18px] my-2 text-[#666]'>
                    <BsFillCalendarDateFill className='text-secondary' />&nbsp; {item.date}
                  </p>
                </div>
                <p className='text-primary text-[24px] cursor-pointer hover:underline'>{item.title}</p>
                <hr className='mt-5' />
        ...
        <div className="w-full md:w-[38%] mt-24 md:mt-0">
          ...
          <div className='w-full mt-10'>
            ...
            <div className='w-full my-5'>
              {listNews.map(i2 => (
                <div key={i2.title} className='w-full flex flex-wrap my-2'>
                  <div className='w-[100px] h-[100px] overflow-hidden relative hidden lg:block'>
                    <img src={i2.picture} className='w-full object-cover absolute top-0 left-0 h-full' alt="" />
                  </div>
                  <div className='w-full bg-gray-200 p-2 lg:bg-white lg:w-[70%] ml-0 md:ml-2'>
                    <p title={i2.title} className='text-[20px] text-primary cursor-pointer hover:underline truncate-text'>{i2.title}</p>
                    <div className='flex items-start w-full justify-start flex-wrap my-3 flex-col'>
                      <p className='flex mr-4 items-center text-[18px] text-[#666]'>
                        <FaUserAlt className='text-secondary' />&nbsp; {i2.author}
                      </p>
                      <p className='flex items-center text-[18px] text-[#666]'>
                        <BsFillCalendarDateFill className='text-secondary' />&nbsp; {i2.date}
                      </p>
                    </div>
                  </div>
                </div>
